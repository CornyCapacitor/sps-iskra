  ...
  const handleCreateCompetition = async (e: { preventDefault: () => void }) => {
    ...
    if (title && description && !file) {
      Swal.fire({
        ...
        background: `${themeBackground}`,
        color: `${themeColor}`,
        title: "Czy na pewno nie chcesz załączać zdjęcia do tego komunikatu z zawodów?",
    ...
    if (title && description && file) {
      Swal.fire({
        ...
        background: `${themeBackground}`,
        color: `${themeColor}`,
        title: "Czy jesteś pewien, że wszystkie pola wypełniłeś poprawnie? Chcesz opublikować tworzony komunikat z zawodów?",
        showConfirmButton: true,
        confirmButtonText: "Tak",
        showCancelButton: true,
        cancelButtonText: "Nie",
  ...
  return (
    <main className="pt-[300px] min-h-screen flex items-start justify-center bg-gray-800 p-6 text-white text-center">
      <div className="bg-gray-900 p-8 rounded-lg shadow-md w-[95%] flex flex-col items-center justify-center gap-5">
        ...
        <button className="w-[350px] p-3 rounded-md bg-green-600 text-white hover:bg-green-700 focus:outline-none text-center" onClick={(e) => handleCreateCompetition(e)}>Stwórz komunikat</button>
        <button className="w-[350px] p-3 rounded-md bg-red-600 text-white hover:bg-red-700 focus:outline-none text-center" onClick={() => abortCompetition()}>Odrzuć wprowadzone zmiany</button>
      </div>
    </main>
  )
}
