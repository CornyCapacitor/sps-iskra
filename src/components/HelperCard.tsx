...
const HelperCard = ({ id, name, path, image }: HelperCardProps) => {
  return (
    <Link href={`${path}`} target="_blank" id={id} className="relative w-[250px] h-[250px] rounded-full p-5 text-black border shadow-1xl hover:cursor-pointer hover:shadow-xl items-center justify-center flex-col transition-[0.2s]">
      {image ? (
        <div className="absolute inset-0 flex items-center justify-center rounded-full">
          <img src={`${properUrl("wspierajacy", id)}`} className="rounded-full" />
        </div>
      ) : (
